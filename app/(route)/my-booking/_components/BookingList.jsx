    ...
    <div>
        {bookingList.length>0?bookingList.map((item,index)=>(
            <div key={index} className=' flex gap-4 items-center border p-5 m-3 rounded-lg'>
                <Image src={item.attributes.service.data.attributes?.image?.data?.attributes?.url}
                ...
                <div className='flex flex-col gap-2 w-full'>
                    <h2 className='font-bold text-[18px] items-center flex justify-between'>{item.attributes.service.data.attributes.Name}
                    ...
                    {item.attributes.service.data.attributes.Address}</h2>
